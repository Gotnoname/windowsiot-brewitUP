...
namespace BrewLib.PID
{
    /// <summary>
    /// https://github.com/ms-iot/pid-controller/blob/master/PidController/PidController
    ...
    public sealed class PIDController
    {
        ...
        public float ControlVariable(long timeSinceLastUpdateMillisec)
        {
            float error = SetPoint - ProcessVariable;

            // integral term calculation
            IntegralTerm += (GainIntegral * error * (float)timeSinceLastUpdateMillisec);
            IntegralTerm = Clamp(IntegralTerm);

            // derivative term calculation
            float dInput = processVariable - ProcessVariableLast;
            float derivativeTerm = GainDerivative * (dInput / (float)timeSinceLastUpdateMillisec);
